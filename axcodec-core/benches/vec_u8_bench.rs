use axcodec_core::{BufferWriter, Decode, Encode, buffer::VecWriter};
...
fn bench_vec_u8_encode(c: &mut Criterion) {
    ...
    for &len in &[0usize, 16, 64, 256, 1024] {
        ...
        group.bench_with_input(BenchmarkId::new("axcodec", len), &vec, |b, v| {
            b.iter(|| {
...
fn bench_vec_u8_decode(c: &mut Criterion) {
    ...
    for &len in &[0usize, 16, 64, 256, 1024] {
        let mut w = VecWriter::new();
        axcodec_core::varint::encode_uvarint(len as u64, &mut w).unwrap();
        w.write_all(&vec![0xABu8; len]).unwrap();
        let encoded = w.into_vec();

        group.bench_with_input(BenchmarkId::new("axcodec", len), &encoded, |b, bytes| {
            b.iter(|| {
                let mut r = axcodec_core::buffer::SliceReader::new(black_box(bytes));
